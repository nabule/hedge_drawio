...
// 自动构建 DrawIO URL（如果未显式配置）
// 优先使用 CMD_DRAWIO_URL，否则基于 CMD_DOMAIN + CMD_DRAWIO_PORT 构建
if (!config.drawio.url && config.domain && config.drawio.port) {
  const protocol = config.protocolUseSSL ? 'https' : 'http'
  config.drawio.url = `${protocol}://${config.domain}:${config.drawio.port}`
}
