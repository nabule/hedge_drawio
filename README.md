...
### 第二阶段目标
1. 完成hedgedoc的编译镜像搭建，采用阶段性编译，减少后面重复编译的时间，必须使用本地 hedgedoc 源码编译，要不然后面修改无法生效。
2. 生成固化镜像编译脚本。
> 注：drawio暂时使用官方镜像，后续需要修改源码时再实现编译。
### 第三阶段目标
1. 集成hedgedoc的drawio插入功能，直接在工具栏增加drawio插入按钮，支持二次修改。
2. 支持以PNG格式插入drawio图形。
### 第四阶段目标
1. 支持以SVG格式插入drawio图形，SVG导出的时候，外观为浅色，选中阴影、嵌入图片、嵌入字体、包含绘图副本选项。 (已完成)
2. 支持配置drawio的导出格式，选项配置。 (已完成)
### 第五阶段目标
...
1. 支持对drawio的文件进行管理（原始文件和渲染后的文件），单独的管理功能/页面，支持统计清理孤立文件及直接下载文件。
   - 访问地址: `/drawio-manager` (需登录)
### 第七阶段目标
1. hedgedoc支持自定义URI，在baseURL后自定义URI即可直接访问对应的文件，如文件不存在就直接新建，如果没有登录就直接新建访客文档。
### 测试要求
每次子任务完成后，必须进行的测试用例。
1. 任务栏功能测试
    1.1. 新建文档
