...
// zip 打包下载
ui.toolbar.download.zip.click(function (e) {
  e.preventDefault()
  e.stopPropagation()
  // 使用服务端路由下载 ZIP
  window.location.href = noteurl + '/download-zip'
})
// export to dropbox
...
/**
 * 绑定 DrawIO 图片点击事件（用于二次编辑）
 * @param {jQuery} container - 包含图片的容器
 */
function bindDrawioImageClickEvents(container) {
  // 查找所有图片
  container.find('img').each(function () {
    ...
    // 检查是否是 DrawIO 图片
    if (src && isDrawioImage(src)) {
      // 添加可编辑样式
      $img.addClass('drawio-editable')
      $img.attr('title', '点击编辑 DrawIO 图形')
      $img.css('cursor', 'pointer')

      // 移除旧的事件监听（防止重复绑定）
      $img.off('click.drawio')

      // 绑定点击事件
      $img.on('click.drawio', function (e) {
        e.preventDefault()
        e.stopPropagation()

        const fileId = getDrawioFileIdFromUrl(src)
        if (!fileId) return
